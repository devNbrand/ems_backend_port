...
export const logWorkHours:any = async (req: Request, res: Response) => {
  try {
    ...
    if (workingHours) {
      ...
      const updatedSessions = await prisma.workSession.createMany({
        data: workSessions.map((session: { startTime: string; endTime: string }) => ({
      ...
      workingHours = await prisma.workingHours.create({
        data: {
          ...
          date: new Date(date),
          workSessions: {
            create: workSessions.map((session: { startTime: string; endTime: string }) => ({
              startTime: new Date(session.startTime),
              endTime: new Date(session.endTime),
            })),
          },
...
export const getWorkHours:any = async (req: Request, res: Response) => {
  try {
    const userId = req.user.id; // Get the user ID from the authenticated user
